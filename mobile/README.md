# Подоконник — мобильное приложение (Flutter, iOS + Android)
...
244 комнатных растения (ароидные, фикусы, драцены, суккуленты и кактусы, марантовые,
...
`supabase/seed.sql`, остальные — миграции `*_knowledge_base_expansion*.sql` (повторный запуск безопасен).
Часть видов добавлена по ассортименту цветочных магазинов (Mayflor, Ok-plants, Unifleur).

У каждого вида есть фото с Wikimedia Commons (с автором и лицензией) и состав грунта — один из
22 рецептов в таблице `soil_mixes` (`care_profiles.soil_mix_slug`) с поправкой под вид, если нужна.

Фото растения в коллекцию только снимается камерой — выбора из галереи нет, и без фото растение
не добавить: так в коллекции и ленте оказываются свои растения, а не картинки из интернета.
